...
def main():
    ...
    # add squares for bmi and ltg
    training_raw['bmi_sq'] = training_raw['bmi'].apply(lambda x: x ** 2)
    training_raw['ltg_sq'] = training_raw['ltg'].apply(lambda x: x ** 2)

    scaler = MinMaxScaler()
    
    # fit_transform: fits the scaler to the data set, and transforms data set
    training_scaled = scaler.fit_transform(
        training_raw[['bmi', 'bmi_sq', 'ltg', 'ltg_sq', 'y']])
    ...
    test_raw['bmi_sq'] = test_raw['bmi'].apply(lambda x: x ** 2)
    test_raw['ltg_sq'] = test_raw['ltg'].apply(lambda x: x ** 2)
    ...
    test_scaled = scaler.transform(test_raw[['bmi', 'bmi_sq', 'ltg', 'ltg_sq', 'y']])
